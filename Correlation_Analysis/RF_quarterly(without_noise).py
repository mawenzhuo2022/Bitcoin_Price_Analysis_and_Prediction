...
def random_forest_prediction(data, feature):
    ...
    daily_info = daily_info[daily_info['Close_Combined_Quarterly'].notna()]

    # Define X (features) and y (target)
    X = daily_info[variables]
    y = daily_info['Close_Combined_Quarterly']  # The current day's close price
    ...
    plt.tight_layout()
    plt.savefig('random_forest_prediction_quarterly_without_noise.png', format='png', dpi=300)
    plt.show()
