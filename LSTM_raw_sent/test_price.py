...
class FinancialPredictor:
    ...
    def prepare_data(self, data, lookback=1):
        """
        Prepare sequences for single target prediction with lagged features
        """
        df = pd.DataFrame(data, columns=['target'])

        # Scale target first
        target_scaled = self.target_scaler.fit_transform(df[['target']])
        df['target_scaled'] = target_scaled

        # Create lagged features from scaled target
        for i in range(1, lookback + 1):
            df[f'lag_{i}'] = df['target_scaled'].shift(i)

        # Drop NaN rows created by shifting
        df = df.dropna()

        # Separate features and target
        X = df[[f'lag_{i}' for i in range(1, lookback + 1)]].values
    ...
    def create_model(self, input_shape, lstm_units=50, dropout_rate=0.2, learning_rate=0.001):
        """
        Updated model to handle the new input shape (lookback, features)
        """
            ...
            for dropout in param_grid['dropout_rate']:
                for lr in param_grid['learning_rate']:
                    ...
                    model = self.create_model(
                        input_shape=(X_train.shape[1], X_train.shape[2]),  # Changed to use proper shape tuple
                        lstm_units=units,
                    ...
                    history = model.fit(
                        X_train,  # Already in correct shape
                        y_train,
                        validation_data=(X_val, y_val),  # Already in correct shape
                        epochs=30,
                        batch_size=32,
    ...
    def evaluate(self, y_true, y_pred):
        # Use target_scaler for inverse transform
        y_true_orig = self.target_scaler.inverse_transform(y_true.reshape(-1, 1))
